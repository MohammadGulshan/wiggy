...
const AppCards = ( card ) => {
    ...
    useEffect(() => { 
        console.log('Cards Mounted!', cards);
        card.getCards();
        setCards(card.card.cards);
    }, [cards, card, card.card.cards]); // passing an empty array as second argument triggers the callback in useEffect only after the initial render thus replicating `componentDidMount` lifecycle behaviour
