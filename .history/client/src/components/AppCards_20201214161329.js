...
const AppCards = ( card ) => {
    ...
    const [state, setState] = useState({
        cards: []
    }); // Here, 'card' is the object passed as parameter to main component and 'cards' is object inside it. so card.cards is assigned to const cards of the current component.

    useEffect(() => { 
        card.getCards();
        console.log('Cards Mounted!', state.cards);
    }, []); // passing an empty array as second argument triggers the callback in useEffect only after the initial render thus replicating `componentDidMount` lifecycle behaviour

    useEffect(() => { 
        console.log('Cards Updated!', state.cards);
        if ( state.cards !== card.card.cards ) {
            setState(state => ({ ...state, cards: card.card.cards }));
        }

    }, [state.cards, card.card.cards]); // passing a second array with dependencies as second argument triggers the callback in useEffect only after the updation of those dependencies thus replicating `componentDidUpdate` lifecycle behaviour
                                            ...
                                            <CardBody>
                                                <CardTitle tag="h5">
                                                    ...
                                                    <Button color="dark" 
                                                        ...
                                                        size="sm"
                                                        onClick={onDeleteCard.bind(this, value._id)}
                                                    >
                                                        X
                                                    </Button>
                                                </CardTitle>
